...
func HandlePostReceive(
	...
	return func(w http.ResponseWriter, r *http.Request) {
		...
		if err != nil {
			log.Ctx(ctx).Error().Err(err).Msg("internal request body in post-receive githook")
		...
		if err != nil {
			log.Ctx(ctx).Error().Err(err).Msg("internal server error in post-receive githook")
