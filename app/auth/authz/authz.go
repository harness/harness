...
type Authorizer interface {
	// Check checks whether the principal of the current session with the provided metadata
	...
	// CheckAll checks whether the principal of the current session with the provided metadata
	...
	// CheckMany checks whether the principal of the current session with the provided metadata
	// has the permission to execute each of the actions on the resource within the scope.
	CheckMany(ctx context.Context, session *auth.Session, permissionChecks ...types.PermissionCheck) ([]bool, error)
}
