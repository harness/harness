...
type PullRequestHandler struct {
	...
	tx                 dbtx.Transactor
	refSyncSpec        RefSyncSpec
}

// RefSyncSpec returns the provider-specific refs needed to mirror one PR.
type RefSyncSpec func(linkedpr.PullRequestPayload) []RefSyncEntry
...
func (h *PullRequestHandler) Handle(
	...
	if existing == nil {
		if err := RunSyncRefs(
			...
			linkedRepo, h.refSyncSpec(prPayload),
		); err != nil {
			return fmt.Errorf("sync refs: %w", err)
		}
		mergeBaseSHA, err := RunMergeBase(
