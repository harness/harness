...
type (
	...
	BuildManager interface {
		...
		Before(ctx context.Context, step *drone.Step) error

		// After signals the build step is complete.
		After(ctx context.Context, step *drone.Step) error
...
// Before signals the build step is about to start.
func (m *Manager) Before(ctx context.Context, step *drone.Step) error {
	logger := logrus.WithFields(
		logrus.Fields{
...
// After signals the build step is complete.
func (m *Manager) After(ctx context.Context, step *drone.Step) error {
	logger := logrus.WithFields(
		logrus.Fields{
			"step.status": step.Status,
			"step.name":   step.Name,
