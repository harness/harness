...
func (s *Service) Merge(ctx context.Context, params *MergeParams) (MergeOutput, error) {
	...
	if baseCommitSHA.IsEmpty() {
		baseCommitSHA, err = s.git.ResolveRev(ctx, repoPath, gitReferenceNamePrefixBranch+params.BaseBranch)
		if err != nil {
			return MergeOutput{}, fmt.Errorf("failed to get base branch commit SHA: %w", err)
		}
	}

	headCommitSHA, err := s.git.ResolveRev(ctx, repoPath, gitReferenceNamePrefixBranch+params.HeadBranch)
	if err != nil {
