...
import (
	...
	url "github.com/harness/gitness/app/url"
...
// PackagePathFor provides a mock function with given fields: ctx, spec
func (_m *Provider) PackagePathFor(ctx context.Context, spec url.PackagePathSpec) (string, error) {
	ret := _m.Called(ctx, spec)

	if len(ret) == 0 {
		panic("no return value specified for PackagePathFor")
	}

	var r0 string
	var r1 error
	if rf, ok := ret.Get(0).(func(context.Context, url.PackagePathSpec) (string, error)); ok {
		return rf(ctx, spec)
	}
	if rf, ok := ret.Get(0).(func(context.Context, url.PackagePathSpec) string); ok {
		r0 = rf(ctx, spec)
	} else {
		r0 = ret.Get(0).(string)
	}

	if rf, ok := ret.Get(1).(func(context.Context, url.PackagePathSpec) error); ok {
		r1 = rf(ctx, spec)
	} else {
		r1 = ret.Error(1)
	}
