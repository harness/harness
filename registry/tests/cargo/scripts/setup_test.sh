...
# Maven conformance test setup script
...
setup_environment() {
  ...
  if [ -z "$space" ]; then
    space="cargo_space_$epoch"
    log "Creating space... $space"
    curl -s -X 'POST' "http://$SERVER_URL/api/v1/spaces" \
      -H 'Content-Type: application/json' \
      -H "Authorization: Bearer $token" \
      -H 'Accept: application/json' \
      -d "{\"description\": \"Cargo conformance test\", \"identifier\": \"$space\",\"is_public\": true, \"parent_ref\": \"\"}"
  fi

  # Create upstream cargo(crates.io) registry if not provided
  if [ -z "$crates_registry" ]; then
    ...
    curl -s -X 'POST' "http://$SERVER_URL/api/v1/registry?space_ref=$space" \
      ...
      -H "Authorization: Bearer $token" \
      -H 'Accept: application/json' \
      -d '{"packageType":"CARGO","identifier":"'$crates_registry'","config":{"type":"UPSTREAM","source":"Crates","authType":"Anonymous","url":"","auth":null},"cleanupPolicy":[],"scanners":[],"parentRef":"'$space'"}'
  else
    log "Using existing registry: $crates_registry"
  fi
  ...
  if [ -z "$registry" ]; then
    ...
    curl -s -X 'POST' "http://$SERVER_URL/api/v1/registry?space_ref=$space" \
      ...
      -H "Authorization: Bearer $token" \
      -H 'Accept: application/json' \
      -d "{\"config\":{\"type\": \"VIRTUAL\", \"upstreamProxies\": [\"$crates_registry\"]}, \"description\": \"\", \"identifier\": \"$registry\", \"packageType\": \"CARGO\",\"parentRef\": \"$space\"}"
  else
  ...
  ENV_FILE="/tmp/cargo_env.sh"
  echo "# cargo test environment variables" > "$ENV_FILE"
...
log "To run tests, use: go test -v ./registry/tests/cargo"
