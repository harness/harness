...
import DartVersionDependencyContent from './DartVersionDependencyContent'
import { DartArtifactDetails, DartArtifactDetailsTabEnum, type DartVersionDetailsQueryParams } from '../../types'

export default function DartVersionArtifactDetailsPage() {
  const { getString } = useStrings()
  const { useUpdateQueryParams, useQueryParams } = useParentHooks()
  const { updateQueryParams } = useUpdateQueryParams()
  const { detailsTab = DartArtifactDetailsTabEnum.ReadMe } = useQueryParams<DartVersionDetailsQueryParams>()

  const { data } = useVersionOverview<DartArtifactDetails>()
  const metadata = data.metadata

  const handleTabChange = useCallback(
    (nextTab: DartArtifactDetailsTabEnum): void => {
  ...
  return (
    <Layout.Vertical padding="large" spacing="large">
      <ButtonTabs small bold selectedTabId={detailsTab} onChange={handleTabChange}>
        <ButtonTab
          id={DartArtifactDetailsTabEnum.ReadMe}
          icon="document"
          iconProps={{ size: 12 }}
          panel={
            <Container>
        ...
        />
        <ButtonTab
          id={DartArtifactDetailsTabEnum.Files}
          icon="document"
          iconProps={{ size: 12 }}
          panel={
        ...
        />
        <ButtonTab
          id={DartArtifactDetailsTabEnum.Dependencies}
          icon="layers"
          iconProps={{ size: 12 }}
          panel={<DartVersionDependencyContent />}
