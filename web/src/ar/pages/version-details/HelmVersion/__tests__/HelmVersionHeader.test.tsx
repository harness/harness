...
import { mockHelmVersionList, mockHelmVersionSummary } from './__mockData__'
...
describe('Verify HelmVersionHeader component render', () => {
  ...
  test('Verify icon, name, selectors and other actions', () => {
    ...
    const data = mockHelmVersionSummary.data
  ...
  test('verify version selector: Success Case', async () => {
    const { container } = render(
      <ArTestWrapper
        ...
        pathParams={{ repositoryIdentifier: 'reg1', artifactType: 'artifacts', artifactIdentifier: 'helm' }}>
        <VersionDetailsPage />
      </ArTestWrapper>
    )
    ...
    await waitFor(() => {
      expect(mockHistoryPush).toHaveBeenLastCalledWith('/registries/reg1/artifacts/helm/versions/1.0.1')
    })
  })
