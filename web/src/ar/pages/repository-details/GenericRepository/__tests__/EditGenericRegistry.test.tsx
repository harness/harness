...
jest.mock('@harnessio/react-har-service-client', () => ({
  ...
  useGetClientSetupDetailsQuery: jest.fn().mockImplementation(() => ({
    ...
    refetch: jest.fn(),
    error: false,
    data: MockGetGenericSetupClientOnRegistryConfigPageResponse
  })),
...
describe('Verify header section for generic artifact registry', () => {
  beforeEach(() => {
    jest.clearAllMocks()
  })
  ...
  test('Verify registry icon, registry name, tag, lables, description and last updated', async () => {
    ...
    const data = MockGetGenericRegistryResponseWithAllData.content.data
...
describe('Verify configuration form', () => {
  ...
  test('should render form correctly with all data prefilled', async () => {
    ...
    expect(nameField).toHaveAttribute('value', MockGetGenericRegistryResponseWithAllData.content.data.identifier)
    ...
    expect(descriptionField).toHaveTextContent(MockGetGenericRegistryResponseWithAllData.content.data.description)

    const tags = registryDefinitionSection.querySelectorAll('div.bp3-tag-input-values .bp3-tag')
    tags.forEach((each, idx) => {
      expect(each).toHaveTextContent(MockGetGenericRegistryResponseWithAllData.content.data.labels[idx])
    })
    ...
    allowedPatterns.forEach((each, idx) => {
      expect(each).toHaveTextContent(MockGetGenericRegistryResponseWithAllData.content.data.allowedPattern[idx])
    })
  ...
  test('should able to submit the form with updated data', async () => {
    ...
    await waitFor(() => {
      expect(modifyRepository).toHaveBeenCalledWith({
        body: {
          ...MockGetGenericRegistryResponseWithAllData.content.data,
