...
import { MockGetGenericRegistryResponseWithAllData } from './__mockData__'
import '../../RepositoryFactory'

const createRegistryFn = jest.fn().mockImplementation(() => Promise.resolve(MockGetGenericRegistryResponseWithAllData))
...
describe('Verify create generic registry flow', () => {
  ...
  test('verify registry type selector', async () => {
    ...
    const genericRegistryTypeOption = dialogBody.querySelector(
      'input[type="checkbox"][name=packageType][value="GENERIC"]'
    )
    expect(genericRegistryTypeOption).not.toBeDisabled()
    fireEvent.change(genericRegistryTypeOption!, { target: { checked: true } })
    expect(genericRegistryTypeOption).toBeChecked()
  })

  test('verify generic registry create form with success scenario', async () => {
    const { container } = render(
      <ArTestWrapper>
        <RepositoryListPage />
      </ArTestWrapper>
    ...
    expect(genericRegistryTypeOption).not.toBeDisabled()
    await userEvent.click(genericRegistryTypeOption!)
    fireEvent.change(genericRegistryTypeOption!, { target: { checked: true } })
    expect(genericRegistryTypeOption).toBeChecked()

    expect(dialogBody).toHaveTextContent('repositoryDetails.repositoryForm.title')

    const formData = MockGetGenericRegistryResponseWithAllData.content.data
    ...
    await waitFor(() => {
      ...
      expect(mockHistoryPush).toHaveBeenCalledWith('/registries/generic-repo/configuration')
    })
  })

  test('verify generic registry create form with failure scenario', async () => {
