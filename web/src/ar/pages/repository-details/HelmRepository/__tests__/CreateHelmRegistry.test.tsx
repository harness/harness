...
import { MockGetHelmRegistryResponseWithAllData } from './__mockData__'
import '../../RepositoryFactory'

const createRegistryFn = jest.fn().mockImplementation(() => Promise.resolve(MockGetHelmRegistryResponseWithAllData))
...
describe('Verify create helm registry flow', () => {
  ...
  test('verify registry type selector', async () => {
    ...
    const helmRegistryTypeOption = dialogBody.querySelector('input[type="checkbox"][name=packageType][value="HELM"]')
    expect(helmRegistryTypeOption).not.toBeDisabled()
    fireEvent.change(helmRegistryTypeOption!, { target: { checked: true } })
    expect(helmRegistryTypeOption).toBeChecked()
  })

  test('verify helm registry create form with success scenario', async () => {
    const { container } = render(
      <ArTestWrapper>
        <RepositoryListPage />
      </ArTestWrapper>
    ...
    const helmRegistryTypeOption = dialogBody.querySelector('input[type="checkbox"][name=packageType][value="HELM"]')
    expect(helmRegistryTypeOption).not.toBeDisabled()
    await userEvent.click(helmRegistryTypeOption!)
    fireEvent.change(helmRegistryTypeOption!, { target: { checked: true } })
    expect(helmRegistryTypeOption).toBeChecked()

    expect(dialogBody).toHaveTextContent('repositoryDetails.repositoryForm.title')

    const formData = MockGetHelmRegistryResponseWithAllData.content.data
    ...
    await waitFor(() => {
      ...
      expect(mockHistoryPush).toHaveBeenCalledWith('/registries/helm-repo/configuration')
    })
  })

  test('verify helm registry create form with failure scenario', async () => {
