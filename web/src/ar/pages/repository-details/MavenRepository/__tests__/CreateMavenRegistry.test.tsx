...
import { MockGetMavenRegistryResponseWithAllData } from './__mockData__'
import '../../RepositoryFactory'

const createRegistryFn = jest.fn().mockImplementation(() => Promise.resolve(MockGetMavenRegistryResponseWithAllData))
...
describe('Verify create maven registry flow', () => {
  ...
  test('verify registry type selector', async () => {
    ...
    const mavenRegistryTypeOption = dialogBody.querySelector('input[type="checkbox"][name=packageType][value="MAVEN"]')
    expect(mavenRegistryTypeOption).not.toBeDisabled()
    fireEvent.change(mavenRegistryTypeOption!, { target: { checked: true } })
    expect(mavenRegistryTypeOption).toBeChecked()
  })

  test('verify maven registry create form with success scenario', async () => {
    const { container } = render(
      <ArTestWrapper>
        <RepositoryListPage />
      </ArTestWrapper>
    ...
    const mavenRegistryTypeOption = dialogBody.querySelector('input[type="checkbox"][name=packageType][value="MAVEN"]')
    expect(mavenRegistryTypeOption).not.toBeDisabled()
    await userEvent.click(mavenRegistryTypeOption!)
    fireEvent.change(mavenRegistryTypeOption!, { target: { checked: true } })
    expect(mavenRegistryTypeOption).toBeChecked()

    expect(dialogBody).toHaveTextContent('repositoryDetails.repositoryForm.title')

    const formData = MockGetMavenRegistryResponseWithAllData.content.data
    ...
    await waitFor(() => {
      ...
      expect(mockHistoryPush).toHaveBeenCalledWith('/registries/maven-repo/configuration')
    })
  })

  test('verify maven registry create form with failure scenario', async () => {
