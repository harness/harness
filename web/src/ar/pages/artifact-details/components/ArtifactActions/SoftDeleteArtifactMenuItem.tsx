...
import useSoftDeleteArtifactModal from '../../hooks/useDeleteArtifactModal/useSoftDeleteArtifactModal'
...
export default function SoftDeleteArtifactMenuItem(props: ArtifactActionProps): JSX.Element {
  ...
  const { triggerSoftDelete } = useSoftDeleteArtifactModal({
    artifactKey,
    onSuccess: handleAfterDeleteRepository,
    uuid: data?.uuid
  })

  const { triggerRestore } = useRestoreDeletedArtifactModal({
    onSuccess: handleAfterDeleteRepository,
    uuid: data?.uuid
  })
