...
function getScopeFromPolicySetRef(policySetRef: string, scope: Scope): { scope: Scope; identifier: string } {
...
export default function useGetPolicySetDetailsPageUrl(policySetRef: string) {
  const { scope } = useAppStore()
  const { routeToMode } = useParentUtils()
  const policySetDetailsScope = getScopeFromPolicySetRef(policySetRef, scope)
  const basePath = routeToMode
    ? routeToMode({
        ...policySetDetailsScope,
        module: 'har'
      })
    : ''
  return `${basePath}/settings/governance/policy-sets/${policySetDetailsScope.identifier}`
}
