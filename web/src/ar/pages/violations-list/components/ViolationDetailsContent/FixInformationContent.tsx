  ...
  return (
    <Layout.Vertical spacing="large">
      <Text font={{ variation: FontVariation.H5, weight: 'bold' }} color={Color.GREY_700}>
        {getString('violationsList.violationDetailsModal.fixInformationSection.title')}
      </Text>
      <Container className={css.gridContainer}>
        <InformationMetrics.Text
          label={getString('violationsList.violationDetailsModal.fixInformationSection.fixAvailable')}
          value={data.fixVersionDetails?.fixVersionAvailable ? 'Yes' : 'No'}
        />
        <div />
        <InformationMetrics.Text
          label={getString('violationsList.violationDetailsModal.fixInformationSection.currentVersion')}
          value={data.fixVersionDetails?.currentVersion?.toString() || getString('na')}
        />
        <InformationMetrics.Text
          label={getString('violationsList.violationDetailsModal.fixInformationSection.fixedVersion')}
          value={data.fixVersionDetails?.fixVersion?.toString() || getString('na')}
        />
      </Container>
    </Layout.Vertical>
  )
...
export default FixInformationContent
