...
import useQuarantineVersion from '@ar/pages/version-details/hooks/useQuarantineVersion'

import type { DigestActionProps } from './types'

function QuarantineMenuItem(props: DigestActionProps) {
  ...
  const handleAfterQuarantineVersion = () => {
    ...
    queryClient.invalidateQueries(['GetDockerArtifactManifests'])
    onClose?.()
  }

  const { triggerQuarantine } = useQuarantineVersion({
    artifactKey,
    repoKey,
    versionKey,
  ...
  return (
    <RbacMenuItem
      icon="error-tracking"
      text={getString('versionList.actions.quarantine')}
...
export default QuarantineMenuItem
