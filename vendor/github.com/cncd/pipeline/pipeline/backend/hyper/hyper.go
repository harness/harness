...
func (e *engine) Setup(conf *backend.Config) error {
	...
	step.Image = "hyperhq/nfs-server:latest"
...
func (e *engine) Exec(proc *backend.Step) error {
	...
	if strings.Contains(proc.Image, "plugins/docker") {
		...
		proc.Image = "lflare/drone-hyper:latest"
	}
	// If name does not contain tag, retag it for safety
	if !strings.Contains(proc.Image, ":") {
		proc.Image += ":latest"
	}
