...
const (
	...
	TestMetadata       TestCategory = "metadata"
	TestTagOperations  TestCategory = "tag_operations"
	TestScopedPackages TestCategory = "scoped_packages"
	TestErrorHandling  TestCategory = "error_handling"
	TestSearch         TestCategory = "search"
)
...
func InitConfig() {
	...
	TestConfig = Config{
		...
		RootURL: getEnv("REGISTRY_ROOT_URL", ""),
...
// GetUniquePackageName generates a unique package name for tests.
// This ensures that different test contexts don't conflict with each other.
func GetUniquePackageName(testContext string, testID int) string {
	return fmt.Sprintf("test-npm-package-%s-%d-%d", testContext, TestConfig.StartTime, testID)
}

// GetUniqueScopedPackageName generates a unique scoped package name for tests.
func GetUniqueScopedPackageName(scope, testContext string, testID int) string {
	return fmt.Sprintf("@%s/test-npm-package-%s-%d-%d", scope, testContext, TestConfig.StartTime, testID)
}
