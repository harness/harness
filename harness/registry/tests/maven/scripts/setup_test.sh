...
setup_environment() {
  ...
  if [ -z "$space" ]; then
    space="maven_space_$epoch"
    log "Creating space... $space"
    curl -s -X 'POST' "http://$SERVER_URL/api/v1/spaces" \
      -H 'Content-Type: application/json' \
      -H "Authorization: Bearer $token" \
      -H 'Accept: application/json' \
      -d "{\"description\": \"Maven conformance test\", \"identifier\": \"$space\",\"is_public\": true, \"parent_ref\": \"\"}"
  fi

  # Create registry if not provided
  if [ -z "$registry" ]; then
    registry="maven_registry_$epoch"
    log "Creating registry: $registry"
    curl -s -X 'POST' "http://$SERVER_URL/api/v1/registry" \
      -H 'Content-Type: application/json' \
      -H "Authorization: Bearer $token" \
      -H 'Accept: application/json' \
      -d "{\"config\":{\"type\": \"VIRTUAL\"}, \"description\": \"\", \"identifier\": \"$registry\", \"packageType\": \"MAVEN\",\"parentRef\": \"$space\"}"
  else
  ...
  ENV_FILE="/tmp/maven_env.sh"
  echo "# Maven test environment variables" > "$ENV_FILE"
...
log "To run tests, use: go test -v ./registry/tests/maven"
