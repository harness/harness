...
func (h *handler) ListPackageVersion(w http.ResponseWriter, r *http.Request) {
	ctx := r.Context()
	...
	response := h.controller.ListPackageVersion(r.Context(), *info)

	if response.GetError() != nil {
		h.HandleError(r.Context(), w, response.GetError())
		return
	}
	err := json.NewEncoder(w).Encode(response.PackageVersion)
	if err != nil {
