...
func (h *handler) GetServiceMetadataV2(w http.ResponseWriter, r *http.Request) {
	ctx := r.Context()
	...
	response := h.controller.GetServiceMetadataV2(r.Context(), *info)

	if response.GetError() != nil {
		h.HandleError(r.Context(), w, response.GetError())
		return
	}
	err := xml.NewEncoder(w).Encode(response.ServiceMetadata)
	if err != nil {
