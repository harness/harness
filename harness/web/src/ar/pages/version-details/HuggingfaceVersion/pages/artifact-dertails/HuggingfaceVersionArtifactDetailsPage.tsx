...
import {
  ...
  type HuggingfaceVersionDetailsQueryParams
} from '../../types'

export default function HuggingfaceVersionArtifactDetailsPage() {
  const { getString } = useStrings()
  const { useUpdateQueryParams, useQueryParams } = useParentHooks()
  const { updateQueryParams } = useUpdateQueryParams()
  const { detailsTab = HuggingfaceArtifactDetailsTabEnum.ReadMe } =
    useQueryParams<HuggingfaceVersionDetailsQueryParams>()

  const { data } = useVersionOverview<HuggingfaceArtifactDetails>()
  const { metadata } = data

  const handleTabChange = useCallback(
    (nextTab: HuggingfaceArtifactDetailsTabEnum): void => {
  ...
  return (
    <Layout.Vertical padding="large" spacing="large">
      <ButtonTabs small bold selectedTabId={detailsTab} onChange={handleTabChange}>
        <ButtonTab
          id={HuggingfaceArtifactDetailsTabEnum.ReadMe}
          icon="document"
          iconProps={{ size: 12 }}
          panel={
            <Container>
        ...
        />
        <ButtonTab
          id={HuggingfaceArtifactDetailsTabEnum.Files}
          icon="document"
          iconProps={{ size: 12 }}
          panel={
