...
import { mockGenericVersionList, mockGenericVersionSummary } from './__mockData__'
...
describe('Verify GenericVersionHeader component render', () => {
  ...
  test('Verify icon, name, selectors and other actions', () => {
    ...
    const data = mockGenericVersionSummary.data
  ...
  test('verify version selector: Success Case', async () => {
    const { container } = render(
      <ArTestWrapper
        ...
        pathParams={{ repositoryIdentifier: 'reg1', artifactType: 'artifacts', artifactIdentifier: 'generic' }}>
        <VersionDetailsPage />
      </ArTestWrapper>
    )
    ...
    await waitFor(() => {
      expect(mockHistoryPush).toHaveBeenLastCalledWith('/registries/reg1/artifacts/generic/versions/1.0.1')
    })
  })
