...
import { MockGetDockerRegistryResponseWithAllData } from './__mockData__'
import '../../RepositoryFactory'

const createRegistryFn = jest.fn().mockImplementation(() => Promise.resolve(MockGetDockerRegistryResponseWithAllData))
...
describe('Verify create docker registry flow', () => {
  ...
  test('verify registry type selector', async () => {
    ...
    const dockerRegistryTypeOption = dialogBody.querySelector(
      'input[type="checkbox"][name=packageType][value="DOCKER"]'
    )
    expect(dockerRegistryTypeOption).not.toBeDisabled()
    fireEvent.change(dockerRegistryTypeOption!, { target: { checked: true } })
    expect(dockerRegistryTypeOption).toBeChecked()
  })

  test('verify docker registry create form with success scenario', async () => {
    const { container } = render(
      <ArTestWrapper>
        <RepositoryListPage />
      </ArTestWrapper>
    ...
    const formData = MockGetDockerRegistryResponseWithAllData.content.data
    ...
    await waitFor(() => {
      ...
      expect(mockHistoryPush).toHaveBeenCalledWith('/registries/docker-repo/configuration')
    })
  })

  test('verify docker registry create form with failure scenario', async () => {
