...
export default function RedirectWidget(props: RedirectWidgetProps): JSX.Element {
  const { factory = repositoryFactory, packageType } = props
  const { getString } = useStrings()
  const repositoryType = factory?.getRepositoryType(packageType)
  if (!repositoryType) {
    return <Text intent="warning">{getString('stepNotFound')}</Text>
  }
  return repositoryType.renderRedirectPage()
}
