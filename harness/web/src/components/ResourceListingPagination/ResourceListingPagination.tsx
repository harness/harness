...
export function useParsePaginationInfo(response: Nullable<Response>) {
  ...
  const page = useMemo(() => parseInt(response?.headers?.get('x-page') || '0'), [response])
  ...
  return { totalItems, totalPages, pageSize, X_NEXT_PAGE, X_PREV_PAGE, page }
}
