...
func provideKubernetesScheduler(config config.Config) core.Scheduler {
	...
	sched, err := kube.FromConfig(kube.Config{
		...
		JobLimitCompute:   config.Kube.JobCPULimit,
		JobLimitMemory:    config.Kube.JobMemoryLimit,
		JobRequestCompute: config.Kube.JobCPURequest,
		JobRequestMemory:  config.Kube.JobMemoryRequest,
